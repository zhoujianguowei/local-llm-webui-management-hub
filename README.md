...
- **全平台支持**：完美兼容 **Windows**, **Linux (Ubuntu)**, **macOS (Apple Silicon)**。
- **完全离线运行**：前端资源 100% 本地化，不依赖任何外网 CDN，满足极高安全要求的内网环境。

### 2. 企业级文件管理
- **智能上传**：支持文件拖拽上传和大文件分片上传，右侧任务栏实时显示每个分片的上传进度和整体百分比。
- **灵活下载**：单个文件直接下载，文件夹自动后台打包为 `.zip` 压缩包，完成后通过下载任务列表通知。
- **多媒体预览**：内置高效预览引擎，无需下载即可查看内容。
  - 图片 (JPG, PNG, GIF, WebP)：支持全屏查看
  - 视频/音频 (MP4, WebM, MP3, WAV)：支持 Range 请求，可随意拖动进度条
  - 文档 (PDF)：内置 PDF.js 阅读器，支持在线预览
  - 文本/代码：支持 Java, Python, JS, HTML, JSON, YAML 等常见格式预览
- **高效搜索**：支持文件名模糊匹配，可使用通配符（如 `*.py`）快速筛选特定类型文件。
...
- **多方式附件上传**：
  ...
  - 文件夹上传：若模型支持图片输入则包含图片文件，否则仅包含纯文本文件
...
- **全量化格式支持**：支持 F32/BF16/F16, Q8_0/Q8_K, Q6_K, Q5_K_M, Q4_K_M, IQ4_NL, IQ3_XXS 等全部量化类型。
- **GGUF 自动化管理**：
...
详细的部署流程、环境配置、权限设置、模型管理、GPU 监控等完整操作指南，请参考项目目录下的 **PDF 文档**。
...
> 💡 **关于商业正式版**：10 天试用期满后，如需继续使用或永久买断（永久全功能激活），请直接在闲鱼拍下对应商品（支持阶梯无损抵扣升级），感谢支持独立开发者！
