...
**🔥 GGUF 模型思考模式自动检测**：通过识别 GGUF 文件内置的 Jinja 模板，自动检测模型是否支持思考模式。已验证支持的模型包括：unsloth 量化的 qwen3.8 系列、gemma4、hy3、deepseek-v4-flash-0731、inkling-small, minimax-m3 等常见开源模型。支持在模型启动时和 AI 对话运行时灵活选择思考等级，无需手动配置。

![主界面](imgs/base_main.png)
![模型启动思考模式](imgs/model-launch-thinking.png)

**⚡ 深度集成 llama.cpp 最新特性**：全面支持多 GPU 并行加速（SM Tensor）与预测推理（Speculative Decoding）加速，包括 MTP (Multi-Token Prediction)、DFlash、DSpark 三种类型，显著提升大模型推理的吞吐量与响应速度。

**🔒 完全离线，开箱即用**：前端资源 100% 本地化，不依赖任何外网 CDN，确保在极高安全要求的内网环境下依然能够流畅运行。

**🚀 零配置部署 & 跨平台支持**：全面兼容 Windows、Linux、macOS 三大主流平台。无需安装与配置数据库，采用单 Jar 包部署模式，真正实现“启动即运行”。

**🛡️ 细粒度路径权限体系**：基于路径继承的权限校验机制，涵盖读取、执行、上传、下载、删除五类核心权限。支持角色最小权限限制及特定用户额外授权，确保私有数据的绝对安全。
...
**⏰ 智能定时重启与维护**：支持一次性及周期性（按工作日）定时关机/唤醒任务。内置任务冲突检测与到期预警功能，是无人值守服务器环境的理想选择。
...
| 版本 | 变更内容 |
| :--- | :--- |
| **v1.12** | 1. 增加 llama.cpp 的 Speculative Decoding 支持，涵盖 DSpark、DFlash 及 MTP 类型，并支持设置 n-max 和 p-min 参数<br />2. AI Chat 功能扩展：支持 Markdown 语法高亮、OpenAI 请求思考内容携带开关，以及运行时动态修改 temperature, top-p 等参数<br />3. OpenAI 请求兼容性增强：由仅支持 llama.cpp 扩展至支持 Ollama, kTransformer, sglang 等主流推理引擎（非 llama.cpp 引擎暂不支持查看 decode/prefill 速度等详细数据）<br />4. 修复文件列表上传、AI Chat 特殊字符转译错误等已知问题 |
| **v1.11** | 1. 修复模型思考模式自动检测bug，新增对于inkling-small、minimax-m3以及deepseek-v4-flash-0731模型思考自动检测<br />2. 修复模型参数导入bug、ai chat页面输出代码渲染问题<br />3. AI Chat 页面 UI 显示优化 |
| **v1.1** | 1. 完全版ai对话支持本地模型自动检测，无需手动配置api，支持模型启动时候或者对话运行时候选择思考等级，自测过unsloth 的qwen3.6，gemma4，deepseek-v4-flash以及hy3等模型<br />2. 新增英语语言支持，支持英语和中文切换<br />3. 增加llama.cpp更多参数支持，包括主gpu、repeat-penalty, top-k, top-p 等参数 <br />4. 修复部分 bug，包括 ai 对话窗口异常关闭提示、路径搜索弹窗高度异常等问题 |
...
### 5. 深度集成 llama.cpp 推理引擎

**也支持添加 ik_llama.cpp 作为推理引擎（非必填）**，如果配置了该路径，模型启动时候，支持选择 llama.cpp 和 ik_llama.cpp 两种推理引擎。ik_llama.cpp 分支在并发处理和部分量化模型加速方面表现更佳，为追求极致性能的极客提供了额外的灵活度。(gguf自动分片底层还是用的是llama.cpp，如果是ik_llama.cpp的分片，可能需要手动合并)
...
- 安装 JDK：[JDK 8 以及以上版本](https://www.oracle.com/java/technologies/downloads/#java8)（推荐 JDK 8 版本，该版本经过多平台测试），并将 Java 可执行路径添加到系统环境变量。Linux 或 Mac 系统需要将 java bin 路径添加到 PATH 路径中。
![JDK环境变量配置](imgs/install-jdk-env.png)

- 安装 [llama.cpp releases](https://github.com/ggml-org/llama.cpp/releases)：根据自身平台下载并进行解压。**对于 NVIDIA 显卡的 Windows 系统，还需要下载对应的 cudart 资源，然后将下载好的资源放入解压后的目录中**（例如 CUDA 版本为 13.3 则下载 cudart 13.3）。
![llama.cpp和cudart安装](imgs/install-llama-cpp-cudart.png)

- 安装 [最新版本的压缩包文件](https://github.com/zhoujianguowei/local-llm-webui-management-hub/releases)：解压后运行对应的脚本即可，可根据配置需要调整 JVM 参数。
![JAR包下载与运行脚本](imgs/install-jar-release.png)
![JAR启动配置](imgs/install-jar-config.png)

- 启动部署并进行基础配置：Jar 包启动完成之后，浏览器打开 [http://localhost:8098/command/static/file/login.html](http://localhost:8098/command/static/file/login.html)。初始账号密码为 `admin` / `admin`，首次登录成功后需要修改账号密码。
![登录页面](imgs/install-login-page.png)

- 授权更新与指南：登录成功后，可以点击 **更新授权** 按钮更新本机授权码。工具栏页面的问号点击进去可以查看详细的使用指南以及功能。
![授权码更新](imgs/install-license-update.png)
![使用指南](imgs/install-help-guide.png)

- 模型管理配置：首次进入模型管理页面，需要填写模型 GGUF 文件所在目录以及 llama.cpp 执行文件所在目录。填写完成后系统会自动识别出模型列表。**模型命名与分片合并** 按钮可以查看详细的 GGUF 文件目录以及命名规范，同时支持 GGUF 分片自动检测合并功能。
![模型管理配置](imgs/install-model-config.png)
![模型命名与分片合并](imgs/install-model-naming-merge.png)

- 配置 GPU 监控：使用 `where nvidia-smi` (Windows) 或 `which nvidia-smi` (Linux/macOS) 查看 `nvidia-smi` 所在系统路径，并添加到模型设置页面。添加完成后即可查看 GPU 信息。
...
- 进行 AI 对话：**v1.1 及以上版本完全版支持本地部署模型自动检测，无需配置 OpenAI API 和模型思考等级参数；若使用 v1.0 或基础版则需要配置 OpenAI API。**
