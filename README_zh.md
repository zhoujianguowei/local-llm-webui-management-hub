...
| **v1.1**  | 1. 完全版ai对话支持本地模型自动检测，无需手动配置api，支持模型启动时候或者对话运行时候选择思考等级，自测过unsloth 的qwen3.6，gemma4，deepseek-v4-flash以及hy3等模型<br />2. 新增英语语言支持，支持英语和中文切换<br />3. 增加llama.cpp更多参数支持，包括主gpu、repeat-penalty、top-k、top-p等参数 <br />4. 修复部分 bug，包括 ai 对话窗口异常关闭提示、路径搜索弹窗高度异常等问题 |
...
- 进行ai对话，**v1.1和以上版本完全版，支持本地部署模型自动检测，无需配置 OpenAI API 和模型思考等级参数，如果是v1.0或者是基础版需要配置 OpenAI API**
